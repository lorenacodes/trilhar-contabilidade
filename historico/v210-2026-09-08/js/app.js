...
/* Navegação compartilhada das páginas novas do cliente (Dashboard, Boletos,
   Documentos, Perfil) — sidebar no desktop/tablet, barra inferior no mobile
   (troca via CSS, o mesmo HTML serve pros dois). Um único ponto de
   manutenção evita que as páginas fiquem com a nav dessincronizada.
   "Enviar Documento" existiu como item próprio até aqui — removido porque
   virou redundante com "Documentos": lá, cada categoria já tem seu próprio
   botão de anexar (pendente/rejeitado/vencido = "Anexar"/"Enviar
   novamente"; qualquer outro status = "Anexar mais um arquivo"), então as
   duas telas faziam basicamente a mesma coisa por caminhos diferentes —
   "Documentos" é a versão melhor porque mostra o status junto. */
const NAV_ITEMS = [
  { slug: "dashboard", label: "Início", icon: "chart", href: "dashboard" },
...
function categoriaVisivelParaCliente(cat, todasCategorias) {
  ...
  // Categoria que virou "pai" (tem subcategoria apontando pra ela via
  // categoria_pai_id) também não aparece -- vira uma pasta organizadora
  // pura, só as subcategorias recebem documento anexado.
  const temFilhos = (todasCategorias || []).some((c) => c.categoria_pai_id === cat.id);
  return !!cat && cat.ativa === true && cat.quem_pode_anexar !== "administrador" && !cat.eh_boleto && !temFilhos;
}
...
/* Rótulo e ordem de exibição de cada frequência de categoria (mesmos valores
   gravados por admin.html em categorias.frequencia — "sob_demanda" é o que
   fica gravado pra categoria não recorrente, não só null). Usado pra
   organizar o checklist do cliente por frequência em vez de por grupo
   (grupo continua existindo só como campo de organização interna do admin). */
...
const FREQUENCIA_ORDEM = ["semanal", "quinzenal", "mensal", "anual", "sob_demanda"];
...
async function renderizarArquivoNoPalco(stageEl, arquivo, downloadBtn) {
  ...
  if (tipo === "PDF") {
    // "Abrir em nova aba" ao lado do iframe -- em celular, o visualizador
    // de PDF embutido no navegador fica espremido dentro do nosso modal
    // (a própria barra de ferramentas/zoom do PDF some ou fica minúscula).
    // Esse link manda pro visualizador de PDF nativo do aparelho, em tela
    // cheia de verdade -- continua funcionando igual no desktop, só que lá
    // já tem espaço de sobra então raramente precisa.
    stageEl.innerHTML = `<iframe src="${data.signedUrl}" title="${escapeHtml(arquivo.nome_original)}"></iframe><a class="doc-viewer-abrir-nova-aba" href="${data.signedUrl}" target="_blank" rel="noopener">${ICONS.download} Abrir em nova aba</a>`;
