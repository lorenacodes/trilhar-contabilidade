...
/* Valida um arquivo contra as regras configuradas na categoria (tipo aceito e
   tamanho máximo) — antes essas regras existiam só na tela de configuração
   de categoria e nunca eram checadas de verdade em nenhum dos 4 pontos de
   upload do sistema. Retorna null se o arquivo passa, ou uma mensagem de erro
   pronta pra mostrar num toast. */
function validarArquivoParaCategoria(file, categoria) {
  const tiposAceitos = (categoria && categoria.tipos_arquivo_aceitos) || [];
  const tamanhoMaximoMb = (categoria && categoria.tamanho_maximo_mb) || 10;
...
function categoriaVisivelParaCliente(cat) {
  ...
  return !!cat && cat.ativa === true && cat.quem_pode_anexar !== "administrador";
}
...
function agruparPorData(itens, obterData) {
  ...
  itens.forEach(item => {
    ...
    const { ano, mes, dia } = chaveDataLocal(iso);
